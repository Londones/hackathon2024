    ...
    async getUserDiseaseData(req, res) {
        try {
            ...
            if (diseaseName.toLowerCase() === 'diabete') {
                return getUserDiabeteDataLast10Days(req, res);
            }
    
            if (diseaseName.toLowerCase() === 'hypertension') {
                return getUserHyperTensionDataThisYear(req, res);
            }
        }
        catch (error) {
            console.log(error);
...
async function getUserDiabeteDataLast10Days (req, res) {
    try {
        const { userId } = req.params;
        const DiseaseModel = getDiseaseModel('diabete');

        const date = moment().subtract(10, 'days').startOf('day').toDate();
...
async function getUserHyperTensionDataThisYear(req, res) {
    try {
        const { userId } = req.params;
